...
class TransferServices {
  ...
  static Future<dynamic> getInvAchBanks() async {
    //Prepare Uri
    var url = Uri.parse(
        '${ApiResources.baseUri! + ApiResources.invAchBanksUri}?ReqMerchantID=${ApiResources.reqMerchantID}&ReqToken=${ApiResources.reqToken}');
    //Send card transfer
    http.Response response;
    try {
      response = await http.get(url, headers: {
  ...
  static Future<dynamic> getInvAch(
      String reqPassword,
      String reqAmount,
      String reqRecipient,
      String reqAccountType,
      String reqBankCode,
      ) async {
    //get CHolderID
    var reqCHolderID = await getCHolderID();
    //Prepare Uri
    var url = Uri.parse(
        '${ApiResources.baseUri! + ApiResources.invAchUri}?ReqMerchantID=${ApiResources.reqMerchantID}&ReqToken=${ApiResources.reqToken}&ReqCHolderID=$reqCHolderID&ReqPassword=$reqPassword&ReqAmount=$reqAmount&ReqRecipient=$reqRecipient&ReqAccountType=$reqAccountType&ReqBankID=$reqBankCode');
    //Send card transfer
    http.Response response;
    try {
      response = await http.get(url, headers: {
